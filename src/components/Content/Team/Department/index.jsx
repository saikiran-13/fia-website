...
const Department = ({ department, members }) => {
    return (
        <div data-aos="fade-up" className='flex flex-col items-center justify-center gap-10'>
            ...
            <div className='flex flex-wrap items-center justify-center'>{members.map(({ name, role, image, ...links }, inx) => <Card key={inx + name} name={name} role={role} image={image} links={links} />)}</div >
        </div>
    )
}
